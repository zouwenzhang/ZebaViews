...
//            return (int) ev.getRawY(pointerIndex);
    ...
    static boolean isConsecutiveScrollerChild(View view) {
        ...
        if (view != null) {
            ...
            if (lp instanceof NestedScrollLayout.LayoutParams) {
                return ((NestedScrollLayout.LayoutParams) lp).isConsecutive;
            }
            return true;
        }
        return false;
    ...
    static boolean isConsecutiveScrollParent(View view) {
        View child = view;
        while (child.getParent() instanceof ViewGroup && !(child.getParent() instanceof NestedScrollLayout)) {
            child = (View) child.getParent();
        }

        if (child.getParent() instanceof NestedScrollLayout) {
    ...
    static boolean isTouchNotTriggerScrollStick(View rootView, int touchX, int touchY) {
        List<NestedScrollLayout> csLayouts = getInTouchCSLayout(rootView, touchX, touchY);
        int size = csLayouts.size();
        for (int i = size - 1; i >= 0; i--) {
            NestedScrollLayout csl = csLayouts.get(i);
            View topView = getTopViewInTouch(csl, touchX, touchY);
            if (topView != null && csl.isStickyView(topView) && csl.theChildIsStick(topView)) {
                NestedScrollLayout.LayoutParams lp = (NestedScrollLayout.LayoutParams) topView.getLayoutParams();
     ...
     * @return
     */
    static List<NestedScrollLayout> getInTouchCSLayout(View rootView, int touchX, int touchY) {
        List<NestedScrollLayout> csLayouts = new ArrayList<>();
        List<View> views = getTouchViews(rootView, touchX, touchY);
        for (View view : views) {
            if (view instanceof NestedScrollLayout) {
                csLayouts.add((NestedScrollLayout) view);
            }
        }
        return csLayouts;
    }

    static View getTopViewInTouch(NestedScrollLayout csl, int touchX, int touchY) {
